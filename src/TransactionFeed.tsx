...
const TransactionFeed = () => {
  ...
  return (
    <div>
      ...
      <div>
        Displaying {displayTxCount} transactions out of {txCount?.txDone} finished transaction and {txCount?.txQueued}{" "}
        queued
      </div>

      {nextTxsReversed != null ? <div className={"tx-33"}>{nextTxsReversed.map(row)}</div> : <div>Loading...</div>}
    </div>
  );
};
